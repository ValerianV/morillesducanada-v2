...
async function sendEmail(payload: {
  ...
  if (!res.ok) {
    ...
    const err = new Error(`Resend error ${res.status}: ${body}`) as Error & { status: number; retryAfterSeconds?: number }
  ...
  for (const queue of ['auth_emails', 'transactional_emails']) {
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      if (payload.queued_at) {
        ...
        if (ageMs > maxAgeMs) {
          console.warn('Email expired (TTL exceeded)', { queue, msg_id: msg.msg_id, queued_at: payload.queued_at })
      ...
      if (payload.message_id) {
        ...
        if (alreadySent) {
          console.warn('Skipping duplicate send', { queue, msg_id: msg.msg_id, message_id: payload.message_id })
